...
export default function Roses() {
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        ...
        if (data.success) {
          const categoryProducts = data.data.filter(p => p.Category === "Roses").slice(0, 3);
  ...
  return (
    <section className="featured-section gold-theme">
      <div className="featured-container">
        <h2 className="featured-title">
          <div className="highlight"></div>
          Roses</h2>
        <div className="featured-subtitle">
          <div className="highlight"></div>
          <span>Timeless elegance in every petal, perfect for romance.</span>
        </div>
